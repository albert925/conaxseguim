<?php
	...
	if (isset($_SESSION['adm'])) {
		...
		if ($iARC=="") {
			echo "<script>";
				echo "alert('Id respondable no disponible');";
				echo "var pag='../tareas';";
...
<body>
	...
	<section>
		<h1>Lista Tareas <?php echo $nommeses[$mesR]." ".$yH; ?></h1>
		<article id="menub">
			...
			<select id="respoS">
				...
				?>
				<option value="<?php echo $idADM ?>" <?php echo $adselec ?>><?php echo "$nomBadm"; ?></option>
				<?php
					}
				?>
			</select>
			...
			<div id="smms" data-msdos="<?php echo $mH ?>" data-msun="<?php echo $mesR ?>" data-year="<?php echo $yH ?>">Sumar mes</div>
		</article>
		<article>
			<?php
				$total_Tareas="SELECT * from tareas where $arjj and fechain_tar>='$fein' and fechain_tar<='$feff'";
				$pendiente_Tareas="SELECT * from tareas where $arjj and esta_tar='1' and fechain_tar>='$fein' and fechain_tar<='$feff'";
				$enproceso_Tareas="SELECT * from tareas where $arjj and esta_tar='2' and fechain_tar>='$fein' and fechain_tar<='$feff'";
				$terminado_Tareas="SELECT * from tareas where $arjj and esta_tar='3' and fechain_tar>='$fein' and fechain_tar<='$feff'";
		...
		<article id="sinscroll">
			<?php
				...
				$ssql="SELECT *	from tareas 
					...
					where $arjj and fechain_tar>='$fein' and fechain_tar<='$feff' 
		...
		<br />
			...
			?>
						<a href="ind2x.php?udad=<?php echo $iARC ?>&ca=<?php echo $mesR ?>&cb=<?php echo $yH ?>&pagina=<?php echo $i ?>"><?php echo "$i"; ?></a>
