...
<body>
	...
	<section>
		...
		<article class="verdatos">
			<div id="informdoiftercer"></div>
			<?php
				$tamno_pagina=12;
				$pagina= $_GET['pagina'];
				if (!$pagina) {
				...
				$ssql="SELECT * from terceros order by nomb_terc asc";
				$rs=mysql_query($ssql,$conexion) or die (mysql_error());
				$num_total_registros= mysql_num_rows($rs);
				$total_paginas= ceil($num_total_registros / $tamno_pagina);

				$mysqlver="SELECT * from terceros order by nomb_terc asc 
				...
				while ($ps=mysql_fetch_array($sql)) {
					...
					$tipoT=$ps['tipo_terc'];
			?>
			<article id="cajP_<?php echo $idT ?>" class="disediver">
				<div>
					<input type="text" id="namef_<?php echo $idT ?>" value="<?php echo $nameT ?>" placeholder="Nombre Tercero" />
				</div>
				<div>
					<input type="tel" id="cednitf_<?php echo $idT ?>" value="<?php echo $cdntT ?>" placeholder="Cédula o Nit" />
				</div>
				<div>
					<input type="tel" id="telff_<?php echo $idT ?>" value="<?php echo $telT ?>" placeholder="Teléfono" />
				</div>
				<div>
					<input type="text" id="direcf_<?php echo $idT ?>" value="<?php echo $dirT ?>" placeholder="Dirección" />
				</div>
			</article>
			<article id="cajP_<?php echo $idT ?>" class="disediver">
				<div>
					<select id="tipof_<?php echo $idT ?>">
						<?php
							for ($x=0; $x <=5 ; $x++) { 
								if ($x==$tipoT) {
									$xx="selected";
				...
				<div id="averl" class="modfiterceros" data-id="<?php echo $idT ?>">Modificar</div>
				<div id="averl">
					<a href="borrarterceros.php?idclE=<?php echo $idT ?>" class="deli">Borrar</a>
				</div>
			</article>
			<?php
				}
		...
		<br />
			...
			?>
						<a href="terceros.php?pagina=<?php echo $i ?>"><?php echo "$i"; ?></a>
