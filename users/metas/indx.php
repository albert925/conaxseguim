<?php
	...
	if (isset($_SESSION['adm'])) {
		...
		$sacar_id_idrec="SELECT * from direcionador where del_admin=$idad";
		$sql_quqerydeldir=mysql_query($sacar_id_idrec,$conexion) or die (mysql_error());
		while ($Rty=mysql_fetch_array($sql_quqerydeldir)) {
			$delidR=$Rty['id_direcion'];
		}
?>
<!DOCTYPE html>
<html lang="es">
	...
	<section>
		...
		<article id="sinscroll">
				<?php
					...
					$YH=date("Y");
					$dia_hoy=$YH."-".$MH."-".$DH;
					$metassql="SELECT * from metab where mes_mtB='$MH' and year_mtB='$YH' and direc_id=$delidR 
						order by id_metB asc";
