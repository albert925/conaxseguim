	...
	<section>
		...
		<article id="verdatos">
			...
			<?php
				...
				$ssql="SELECT * from ciudad order by nam_ciudad asc";
				$rs=mysql_query($ssql,$conexion) or die (mysql_error());
				$num_total_registros= mysql_num_rows($rs);
				$total_paginas= ceil($num_total_registros / $tamno_pagina);

				$mysqlver="SELECT * from ciudad order by nam_ciudad asc limit $inicio, $tamno_pagina";
				$sql=mysql_query($mysqlver,$conexion) or die (mysql_error());
				while ($ps=mysql_fetch_array($sql)) {
					$idC=$ps['id_ciudad'];
					$naC=$ps['nam_ciudad'];
			?>
			<article id="cajP_<?php echo $idC ?>" class="disediver">
				<div>
					<input type="text" id="ciudf_<?php echo $idC ?>" value="<?php echo $naC ?>" placeholder="Pais" />
				</div>
				<div id="averl" class="CIC" data-id="<?php echo $idC ?>">Modificar</div>
				<div id="averl"><a href="borarciudad.php?idCEv=<?php echo $idC ?>" class="deli">Borrar</a></div>
			</article>
			<?php
				}
			?>
		...
		<br />
			...
			?>
						<a href="ciudad.php?pagina=<?php echo $i ?>"><?php echo "$i"; ?></a>
