	...
	<section>
		...
		<article id="verdatos">
			...
			<?php
				...
				$mysqlver="SELECT * from departamento order by nam_depart asc limit $inicio, $tamno_pagina";
				$sql=mysql_query($mysqlver,$conexion) or die (mysql_error());
				while ($ps=mysql_fetch_array($sql)) {
					$idD=$ps['id_depart'];
					$naD=$ps['nam_depart'];
			?>
			<article id="cajP_<?php echo $idD ?>" class="disediver">
				<div>
					<input type="text" id="departf_<?php echo $idD ?>" value="<?php echo $naD ?>" placeholder="Depart/Estad" />
				</div>
				<div id="averl"><a href="relacionSeliB.php?IdDepart=<?php echo $idD ?>">Ver relación</a></div>
				<div id="averl" class="modifDE" data-id="<?php echo $idD ?>">Modificar</div>
				<div id="averl"><a href="borardepart.php?idDEv=<?php echo $idD ?>" class="deli">Borrar</a></div>
			</article>
			<?php
				}
			?>
		...
		<br />
			...
			?>
						<a href="departamentoestado.php?pagina=<?php echo $i ?>"><?php echo "$i"; ?></a>
